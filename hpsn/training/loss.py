"""Masked-reconstruction loss (masked frames only)."""
...
class HPSNLoss(nn.Module):
    def __init__(self, lambda1: float = 1.0, lambda2: float = 1.0, loss_type: str = "l1"):
        super().__init__()
    ...
    def forward(self, outputs: dict) -> dict:
        ...
        total = self.lambda1 * loss1 + self.lambda2 * loss2
        return {"total": total, "recon1": loss1.detach(), "recon2": loss2.detach()}
